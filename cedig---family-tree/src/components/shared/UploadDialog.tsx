...
export function UploadDialog({ open, onClose, type, onUpload }: UploadDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-[140] bg-black/75 backdrop-blur-md flex items-center justify-center p-4" onClick={onClose}>
      <motion.div
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#32231A]/20">
          <div>
            ...
            <p className="text-[10px] text-ink/50 mt-0.5">
              {isPhoto ? "Add a photograph to the archival record" : "Upload a document (PDF, image)"}
            </p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-full hover:bg-stone-100 transition-colors cursor-pointer">
        ...
        <div className="p-5 space-y-4">
          <div
            ...
            className={cn(
              ...
              isDragging ? "border-bronze bg-bronze/10" : selectedFileUrl ? "border-bronze/50" : "border-ink/20 hover:border-bronze/50",
            )}
          >
            <input
              ref={fileInputRef}
            ...
            {selectedFileUrl && isPhoto ? (
              <div className="relative h-24 mx-auto max-w-[120px] rounded-lg overflow-hidden">
                <Image src={selectedFileUrl} alt="Preview" fill sizes="120px" className="object-cover" />
              </div>
            ) : (
              <div className="flex flex-col items-center">
                <div className="w-12 h-12 rounded-full bg-bronze/15 flex items-center justify-center mb-2">
                  {isPhoto ? <ImageIcon className="w-6 h-6 text-bronze" /> : <FileText className="w-6 h-6 text-bronze" />}
                </div>
                <Upload className="w-5 h-5 text-ink/30 mb-1" />
                <p className="text-xs font-bold text-ink/50">
                  {isPhoto ? "Drop a photo here" : "Drop a document here"}
                </p>
                <p className="text-[10px] text-ink/30 mt-0.5">or click to browse</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder={isPhoto ? "e.g., Summer of 1964, Ulaanbaatar..." : "Brief description..."}
              className="w-full bg-white border border-ink/15 rounded-xl py-2.5 px-3 text-xs focus:outline-none focus:border-bronze placeholder-ink/30"
            />
          </div>

          <div className="flex gap-2 pt-2">
            <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-stone-200 text-ink text-xs font-bold hover:bg-stone-300 transition-colors cursor-pointer">
              Cancel
            </button>
            <button
